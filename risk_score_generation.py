...
def save_tmp_csv(temp_df):
    """Function to save only datetime, source, and risk columns as temporary date_risk.csv for aggregation stage.

    Args:
        temp_df (pandas DataFrame): json_data loaded as pandas df.

    Returns:
        temp_date_risk.csv file with columns 'datetime', 'source', 'risk score'
    """
    df_new = temp_df[["datetime", "source", "risk score"]]
...
def append_score_to_csv(json_data, risk_scores, filename):
    ...
    try:
        ...
        if len(risk_scores) == len(temp_df):
            ...
            # Save only datetime, source, and risk columns to temp_date_risk.csv for aggregation step.
